...
import { changeText, cleanSearchBar } from "./filterSortingSlice"; // Make sure these actions exist

export const ProductSearchBar = () => {
    const { searchText, isFixed } = useSelector(
        (store: RootState) => store.filterSorting // Assuming you're using the same slice
    );
        ...
        >
            <input
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    dispatch(changeText(e.target.value)) // Change the action to match product searching
                }
                value={searchText}
          ...
          text-xl w-full focus:outline-none h-full font-light"
            ...
            {searchText.length > 0 && (
                <button
                    ...
                    onClick={() => dispatch(cleanSearchBar())} // Change to match product searching
                >
                    <FaXmark />
                </button>
            )}
